...
parser.add_argument("--coverages_dir",
                    dest="coverages_dir",
                    help=("directory with pickle files that " +
                        "store the HTSeq objects with coverages") )

parser.add_argument("--conditions",
                    dest="conditions",
                    nargs="*",
                    help=("values that match the sample_names " +
                        " and indicate the condition each " +
                        "sample belongs to") )
...
parser.add_argument("--ds_extension", 
                    ...
                    type=int, 
                    default=200,
                    help="<INT> number of nucleotides that were appended to the exon end to create coverage [ default: 200]")
...
parser.add_argument("-d",
                    ...
                    action="store_true",
                    default=True,
                    help="Print additional diagnostic comments")

parser.add_argument("--ex_extension_dir",
                    dest="ex_extension_dir",
                    help=("directory with the files that " +
...
def find_distal_site(site,
    ...
    if strand == "+":
        ...
        if ex_start + min_coverage_region > site[1].start-read_length:
            # consistency check:
            # is exon start more upstream than site start
            if site[1].start < ex_start+min_coverage_region:
            ...
            ex_start_2_idx = region.index(ex_start+ min_coverage_region)
        else:
            ex_start_2_idx = region.index(ex_start + min_coverage_region)
    else:
        if ex_start - min_coverage_region < site[1].end+read_length:
            # consistency check:
            # is exon start more upstream than site start (meaning the strand specific starts)
            if site[1].end >= ex_start- min_coverage_region:
                syserr("[INFO] Distal site search stopped whith the current site %s that overlaps the exon start\n"
                       % site[0] )
                return(None, -1)
            ...
            # exon is very short: use the start of the currently investigated
            # poly(A) site as boundary
            ex_start_idx = region.index( ex_start - min_coverage_region)
        else:
            ex_start_idx = region.index(ex_start - min_coverage_region)
    ...
    ex_start_cov = np.median(merged_exons[ ex_start_idx:ex_start_2_idx ])
    ...
    ex_us_cov = np.median(merged_exons[ ex_us_idx:ex_us_2_idx ])
    ...
    if strand == "+":
        ...
        ex_ds_cov = np.median( merged_exons[ ex_ds_idx:ex_ds_2_idx] )
    else:
    ...
    ####
    # Aleksei: we remove checking for ex_us_cov > ex_ds_cov as it is nowhere documented, plus 2*read_length is not controlled.
    # Plus, it is simply wrong for the clusters of closely located PAS sites 
    ####   
    # if( ex_us_cov > ex_ds_cov and
    #     ex_ds_cov / float(ex_start_cov) * 100 <= max_downstream_coverage):
    if( ex_ds_cov / float(ex_start_cov) * 100 <= max_downstream_coverage):
        # distal site found
        return(1, 1)
    else:
...
def assess_minimum_cvg(exon_structure, bp_coverages_dict, region, min_mean_cvg):
    ...
    for cond in bp_coverages_dict:
        ...
        for cvg in bp_coverages_dict[cond]:
            if np.median( cvg[start_idx:end_idx] ) < min_mean_cvg:
...
def find_best_break_point_in_full_exon(bp_cvg_dict,
    ...
    for cond in bp_cvg_dict:
        ...
        for cvg_idx in range(len(bp_cvg_dict[cond])):
            ...
            global_mse = np.median( np.abs(cvg_global - np.median(cvg_global)) )
            ...
            for pos_idx in range(upstream_start_idx + min_cvg_reg + 1, downstream_end_idx - min_cvg_reg - 1):
                ...
                cov_diff = cvg_left - np.median(cvg_left)
                cov_diff = np.append( cov_diff, (cvg_right - np.median(cvg_right) ) )
                curr_mse = np.median( np.abs(cov_diff) )
                if global_mse==0:
                    mse_ratio = 10
                else:
...
def check_mse_improvement(bp_coverages_dict,
    ...
    for cond in bp_coverages_dict:
        ...
        for cvg in bp_coverages_dict[cond]:
            ...
            for pos in fit_region_to_iterate:
                ...
                mean_upstream = np.median( cvg_upstream )
                ...
                mean_downstream = np.median( cvg_downstream )
    ...
    for cond in mse_ratio_dict:
        ...
        if len(mse_ratio_subset) == 0:
            ...
            if sum( mse_ratio_subset < mse_ratio_threshold) >= 1: 
                valid_conds.append(cond)

        ###
        # Aleksei: if at least in one sample from the condition the ration is valid, we call the whole condition as valid
        # Otherwise, it makes the procedure non-optimal for the case of hundreds of samples from the same condition (which are actually not the same, but this is to be inferred afterwards)
        ###        
    ...
    if len(valid_conds) > 0:
        ...
        for cond in bp_coverages_dict:
            ...
            for cvg in bp_coverages_dict[cond]:
                ...
                cvg_upstream = cvg[up_start_idx:up_end_idx]
                mean_upstream = np.median(cvg_upstream)
                us_mse = np.median(np.abs(cvg_upstream - mean_upstream))

                cvg_downstream = cvg[down_start_idx:down_end_idx]
                mean_downstream = np.median( cvg_downstream )
                ds_mse = np.median(np.abs(cvg_downstream - mean_downstream))
...
def find_sites_with_usage(sites,
    ...
    for site_idx in range(len(sites)):
        site = sites[site_idx]
        # Aleksei: check Dicer
        if segment_start==108043088 and segment_end==108047292:
            syserr("[INFO] "+str(site)+" site is currently considered\n")
        
        # if the site is actually a cluster of closely located PAS
        if isinstance(site[0], list):
            ...
            # Aleksei: check Dicer
            if segment_start==108043088 and segment_end==108047292:
                syserr("[INFO] "+str(extended_segment_mse)+" is extended segment MSE for site "+str(sites[site_idx])+"\n")
            site = sites[site_idx]
            (mse_ratios, 
             us_mse_dict,
             ds_mse_dict,
            ...
            # Aleksei: check Dicer
            if segment_start==108043088 and segment_end==108047292:
                syserr("[INFO] "+str(mse_ratios)+" are MSE ratios for site "+str(site)+", MSE thr: "+str(mse_ratio_threshold)+"; valid cond list: "+str(valid_conds_list)+"\n")            
            ...
            if original_extended_segment_mse is not None:
                ...
                for cond in bp_coverages_dict:
                    ...
                    for cvg_idx in range(len(bp_coverages_dict[cond])):
                        ...
                        ext_seg_mean = np.median( extended_y )
                        ext_seg_diff = extended_y - ext_seg_mean
                        ext_seg_mse = np.median( np.abs(ext_seg_diff))
                ...
                extended_segment_mse = None       
        ###
        # Aleksei: here, it seemed like a bug that 2 was subtracted instead of one. Remember that sites already do not contain a distal site.
        ###
        
        if top_site_idx < len(sites) - 1:
...
def get_pA_set(exon_id, 
    ...
    # Aleksei: check Dicer
    if exon_id=='ENSG00000196776.17:11:11:108043091:108047292':
        syserr("[INFO] "+str(distal_site)+" is distal site in exon "+exon_id+"\n")
    ...
    # Aleksei: check Dicer
    if exon_id=='ENSG00000196776.17:11:11:108043091:108047292':
        syserr("[INFO] "+str(sum([i for lst in valid_mean_cvg_dict.values() for i in lst]))+" samples with sufficient coverage for "+exon_id+"\n")
        syserr("[INFO] "+str(len(polyA_sites))+" non-distal PAS for "+exon_id+"\n")
        syserr("[INFO] "+str(exon_structure)+" is exon structure for "+exon_id+"\n")
    ...
    if len(polyA_sites) == 0:
        ...
        ###
        # Aleksei: Here, polyA_sites is empty and this will later cause "No site was inferred for <>. Skipped exon!"
        # Distal site was excluded just before.
        ###        
        
        polyA_sites.append(distal_site)
    ...
    # Aleksei: check Dicer
    if exon_id=='ENSG00000196776.17:11:11:108043091:108047292':
        syserr("[INFO] "+str(used_sites)+" are used sites for "+exon_id+"\n")
    ...
    if len(used_sites) > 1:
        ...
        for cond in bp_coverages_dict:
            for cvg_idx in range(len(bp_coverages_dict[cond])):
                ...
                ###
                # Aleksei: turn off this filtration, as it seems to perform very weirdly
                ###
...
def get_distal_expression(used_sites,
    ...
    if exon_structure[3] - read_length - exon_structure[2] < min_coverage_region:
        syserr("[INFO] Single distal site of exon "+str(exon_id)+" is very close to exon start. " + 
    ...
    for cond in bp_coverages_dict:
        ...
        for cvg_idx in range(len(bp_coverages_dict[cond])):
            ...
            ###
            # Aleksei: turn off this filtration, because the fact that there is a valid break point can arise due to mild (high) mse_ratio_threshold
            # high mse_ratio_threshold allows to quantify more PAS, but it also discourages the quantification of singular PAS expression due to valid_break_point_found check.
            # Hence, many singular PAS have -1 as expression, although they could be quantified 
            ###            
            ...
            cvg = bp_coverages_dict[cond][cvg_idx]
            mean_cvg = np.median( cvg[start_idx:end_idx] )
        ...
        for cvg in bp_coverages_dict[cond]:
            ...
            if strand == "+":
                ...
                try:
                    ...
                    start_idx = region.index( exon_structure[3] + add_us_extension[cond][cvg_counter] - 2) + 1
            ...
            if (valid_mean_cvg_dict[cond][cvg_counter] == 0 or
                ...
                y = cvg[start_idx:end_idx]
                region_means[cond][cvg_counter].append(np.median(y))
        ...
        for cond in bp_coverages_dict:
            ...
            for cvg in bp_coverages_dict[cond]:
                ...
                if (valid_mean_cvg_dict[cond][counter] == 0 or
                    ...
                    y = cvg[start_idx:end_idx]
                    region_means[cond][counter].append(np.median(y))
        ...
        for cond in region_means:
            for sample_arr_idx in range(len(region_means[cond])):
                if (valid_mean_cvg_dict[cond][sample_arr_idx] == 0 or
                    valid_mean_cvg_dict[cond][sample_arr_idx] == -1):
                    continue
                if sum(ds_corrected_means[cond][sample_arr_idx])<0:
                    downstream_corrected_mean = region_means[cond][sample_arr_idx][site_idx]
                else:
    ...
    if len(used_sites) > 1:
        ...
        if len(relative_usage_dict) > 0:
            ...
            for site_idx in range(len(used_sites)):
                ...
                for file_idx in range(len(options.sample_names)):
...
def main(options):
    ...
    syserr("[INFO] "+str(len(term_ex_us_ex_dict))+" terminal exons are collected from input.\n") # Aleksei        
    ...
    for exon in terminal_exons_dict:
        ...
        for cl_idx in range(1, len(polyA_sites)):
            prox_site = polyA_sites[cl_idx -1] # actually, this is a cluster, not the site
            dist_site = polyA_sites[cl_idx] # actually, this is a cluster, not the site
            ...
            (strand == "-" and (prox_site[1].start - dist_site[1].end) < options.min_cluster_distance) ):
                ...
                if (cl_idx - 1) in close_cluster_idxs:
                    # simply update (add the elem to) the set of close clusters
                    ...
                    if len(close_cluster_idxs) > 0:
                        if len(close_cluster_idxs) < 2:
                            syserr("[ERROR] set of closely spaced sites has 1 entry (%s) for exon %s\n" % (str(close_cluster_idxs), exon))
                            sys.exit(2) 
                        list_of_close_clusters.append(sorted(close_cluster_idxs))
                        
                        syserr("[INFO] PAS "+', '.join([str(elem) for elem in list(sorted(close_cluster_idxs))])+" are found to be closely spaced for exon "+str(exon)+".\n") # Aleksei
                        close_cluster_idxs = set()
                    close_cluster_idxs.update([(cl_idx-1), cl_idx])
        # empty set if necessary since we iterated to the end
        if len(close_cluster_idxs) > 0:
            list_of_close_clusters.append(sorted(close_cluster_idxs))
            syserr("[INFO] PAS "+', '.join([str(elem) for elem in list(sorted(close_cluster_idxs))])+" are found to be closely spaced for exon "+str(exon)+".\n") # Aleksei
            
        if len(list_of_close_clusters) > 0:
            syserr("[INFO] "+str(len(list_of_close_clusters))+" closely spaced clusters of PAS were identifed for exon "+str(exon)+".\n") # Aleksei
            new_pA_list = []
            last_processed_pA = 0
    ...
    coverage_pkl_dict = {}
    coverages_directory = options.coverages_dir
    cov_file_path_list = [os.path.join(coverages_directory, file) for file in os.listdir(coverages_directory)]
    ex_extension_directory = options.ex_extension_dir
    ext_file_path_list = [os.path.join(ex_extension_directory, file) for file in os.listdir(ex_extension_directory)]

    k = 0
    for sample_id in options.sample_names:
        pkl_idx = k
        pkl_file = ''
        for cov_file_path in cov_file_path_list:
            if sample_id in cov_file_path and cov_file_path.endswith(".pkl"):
                pkl_file = cov_file_path
                sample_basename = pkl_file.replace(".pkl", "")
                sample_basename = os.path.basename(sample_basename)

                extension_file = ''
                for tmp_file in ext_file_path_list:
                    if sample_id in tmp_file and tmp_file.endswith("extensions.tsv"):
                        extension_file = tmp_file
                if extension_file=='':
                    syserr("[ERROR] no extension file found for " +sample_basename+"\n")
                    sys.exit(2)
                break
        if pkl_file=='':
            syserr("[ERROR] no pkl file found for " +sample_id+"\n")
            sys.exit(2)            
    ...
    for exon in terminal_exons_dict:
        ...
        syserr("[INFO] for exon "+str(exon)+" max upstream extension is "+str(max_ex_upstream_ext)+" .\n") # Aleksei
        ...
        if strand == "+":
            if isinstance(polyA_sites[-1][0], list) and polyA_sites[-1][-1][1].end > end:
                ...
                syserr("[INFO] extended 3' end for exon "+str(exon)+" to pos "+str(start)+".\n") # Aleksei
        ...
        if strand == "+":
            ### Aleksei:
            # region = range( (start - max_ex_upstream_ext ), (end + options.ds_ext) )
            region = range( (start - max_ex_upstream_ext ), (end + options.ds_reg_for_no_coverage) )
        else:
            # region = range( (start - options.ds_ext), (end + max_ex_upstream_ext) )
        ...
        # merged_exons = np.zeros(end - start + options.ds_ext + max_ex_upstream_ext)
        ...
        for cond in coverage_pkl_dict:
            ...
            for cvg in coverage_pkl_dict[cond]:
                ...
                if np.mean(curr_cov_arr)>options.min_mean_coverage: # Aleksei: mean should be better here than median 
                    curr_cov_arr = curr_cov_arr / float(np.max(curr_cov_arr)) * 100

...
parser.add_argument("--extensions_out",
                    dest="extensions_out",
                    help="name TSV extensions output file. The name for the wiggle files is inferred from the pickle name (suffix replacement).")
...
def process_single_read(aln,
    ...
    if splice_candidate:
        ...
        ### Aleksei: comment out the following section
        ...
        ### Aleksei: define the left-most and right-most borders as fragment start and end
        ### add the coverage for the whole insert region, not only for sequenced parts
            ...
            if len( reg[1] ) == 1:
                if(reg[0].start > region.end or
                   reg[0].end < region.start):
                    ...
                    cur_chrom = reg[0].chrom
                    cur_start = reg[0].start
                    cur_end = reg[0].end
                    cur_strand = reg[0].strand
                    if min_start==-1:
                        min_start = cur_start
                    else:
        ...
        if min_start!=-1 and max_end!=-1:
            fragment_iv = HTSeq.GenomicInterval( cur_chrom, min_start, max_end, cur_strand )
            cvg[ fragment_iv ] += read_weight
...
def infer_exon_start(bam, region_string, region_start, region_end, strand, unstranded):
    ...
    for aln in bam.fetch(region = region_string):
        ...
        if not unstranded:
            ...
            if not ((aln.pe_which == "first" and aln.iv.strand == opp_strand) or
                (aln.pe_which == "second" and aln.iv.strand == strand)):
                continue
            if not (aln.pe_which == 'not_paired_end' and aln.iv.strand == strand):
                    continue
...
def count_same_name_reads(same_read_list,
    ...
    if len(same_read_list) == 1:
            ...
            for aln_idx in range(len(same_read_list)):
                ...
                if curr_mate_name in mate_name_dict:
                    ...
                    if( (not unstranded and curr_aln.pe_which == "first") or
                        (unstranded and curr_aln.iv.strand == opp_strand)):
                        ...
                        for cig_op in curr_aln.cigar:
                            ...
                            ### Aleksei - begin
                            if not (((cig_op.ref_iv.start < reg_start) and (strand=='+')) or ((strand=='-') and (cig_op.ref_iv.end > reg_end))):
                    ...
                    if min_start!=-1 and max_end!=-1:
                        fragment_iv = HTSeq.GenomicInterval( cur_chrom, min_start, max_end, cur_strand )
                        cvg_tmp[ fragment_iv ] += read_weight
        ...
        for a in same_read_list:
            ...
            if unstranded is True:
                reverse_strand = True if a.iv.strand == opp_strand else False
            else:
...
def get_coverage(bam,region_string, strand, unstranded):
    ...
    for new_aln in aln_list:
        ...
        #     opp_strand = ('+' if strand=='-' else '-')
        # # skip alignment if it has the wrong orientation
        #     if not ((new_aln.pe_which == "first" and new_aln.iv.strand == opp_strand) or
        #         (new_aln.pe_which == "second" and new_aln.iv.strand == strand)):
        #         continue
        #     ### Aleksei: for single-end library forward orientation is assumed 
        #     if not (new_aln.pe_which == 'not_paired_end' and new_aln.iv.strand == strand):
        #             continue
...
def process_same_name_reads(same_read_list,
    ...
    if len(same_read_list) == 1:
        ...
        aln = same_read_list[0]
        if unstranded is True:
            reverse_strand = True if aln.iv.strand == opp_strand else False
        else:
            reverse_strand = True if aln.pe_which == "first" else False
...
def main(options):
    ...
    syserr("[INFO] Number of exons: " +str(len(exons))+'\n')
    syserr("[INFO] Number of exons for which the exon end was extended because of the poly(A) site coordinates: %i\n" % (adjusted_ex_coords))
    ...
    with open( options.extensions_out, "w") as ext_out:
